...
const TABLA = 'proyecto_linea';
const VACIO = { proyecto: '', linea_investigacion: '' };

export default function ProyectoLinea() {
  ...
  useEffect(() => {
    cargar();
    Promise.allSettled([listar('proyecto'), listar('linea_investigacion')])
      .then(([rP, rL]) => {
        if (rP.status === 'fulfilled') setProyectos(rP.value.data.datos || []);
        if (rL.status === 'fulfilled') setLineas(rL.value.data.datos || []);
      });
  }, []);

  const handleChange     = (e) => setForm({ ...form, [e.target.name]: e.target.value });
  const formularioValido = form.proyecto !== '' && form.linea_investigacion !== '';
  ...
  const handleGuardar = async () => {
    ...
    try {
      await crear(TABLA, { proyecto: parseInt(form.proyecto), linea_investigacion: parseInt(form.linea_investigacion) });
      await cargar();
      cerrarModal();
    } catch (e) {
  ...
  const handleEliminar = async (fila) => {
    try {
      await eliminar(TABLA, 'proyecto', `${fila.proyecto}/linea_investigacion/${fila.linea_investigacion}`);
      await cargar();
    } catch (e) {
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h2>Líneas de Investigación por Proyecto</h2>
          <p>Relación entre proyectos y líneas de investigación</p>
        </div>
      ...
      <div className="table-wrapper">
        ...
        : (
          <table>
            <thead>
              <tr><th>Proyecto</th><th>Línea de Investigación</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              {datos.map((fila, i) => (
                <tr key={i}>
                  <td>{nombreProyecto(fila.proyecto)}</td>
                  <td>{nombreLinea(fila.linea_investigacion)}</td>
        ...
        >
          ...
          <div className="form-grid">
            ...
            <div className="form-group full">
              ...
              <select name="linea_investigacion" value={form.linea_investigacion} onChange={handleChange}>
                <option value="">— Seleccionar —</option>
