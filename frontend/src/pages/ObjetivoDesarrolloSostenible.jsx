...
const TABLA = 'objetivo_desarrollo_sostenible';
const PK    = 'id';
const VACIO = { id: '', nombre: '' };

export default function ObjetivoDesarrolloSostenible({ readonly = false }) {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p>Gestión de objetivos de desarrollo sostenible (ODS)</p>
        </div>
        {!readonly && <button className="btn-primary" onClick={abrirCrear}>+ Nuevo ODS</button>}
      </div>
      ...
      <div className="table-wrapper">
        ...
        : datos.length === 0 ? <p className="table-empty">No hay ODS registrados aún.</p>
        : (
          <table>
            <thead>
      ...
      {modal && (
        <Modal titulo={modal === 'crear' ? 'Nuevo ODS' : 'Editar ODS'} onClose={cerrarModal}>
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              <label>Nombre</label>
              <input name="nombre" type="text" value={form.nombre} onChange={handleChange} placeholder="Nombre del ODS" />
            </div>
          </div>
      ...
      {confirmEliminar !== null && (
        <Modal titulo="Confirmar eliminación" onClose={() => setConfirmEliminar(null)}>
          <p style={{ color: '#cbd5e1', fontSize: '14px' }}>
            ¿Estás seguro de eliminar este ODS? Esta acción no se puede deshacer.
          </p>
          <div className="modal-footer">
