...
const TABLA = 'ac_proyecto';
const VACIO = { proyecto: '', area_conocimiento: '' };

export default function AcProyecto() {
  ...
  useEffect(() => {
    cargar();
    Promise.allSettled([listar('proyecto'), listar('area_conocimiento')])
      .then(([rP, rA]) => {
  ...
  const formularioValido = form.proyecto !== '' && form.area_conocimiento !== '';
  ...
  const handleGuardar = async () => {
    ...
    try {
      await crear(TABLA, { proyecto: parseInt(form.proyecto), area_conocimiento: form.area_conocimiento });
      await cargar();
      cerrarModal();
    } catch (e) {
  ...
  const handleEliminar = async (fila) => {
    try {
      await eliminar(TABLA, 'proyecto', `${fila.proyecto}/area_conocimiento/${fila.area_conocimiento}`);
      await cargar();
    } catch (e) {
  ...
  const nombreArea     = (id) => areas.find(a => a.id == id)?.disciplina || id;

  return (
    <div>
      <div className="page-header">
        <div>
          <h2>Áreas de Conocimiento por Proyecto</h2>
          <p>Relación entre proyectos y áreas de conocimiento</p>
        </div>
      ...
      <div className="table-wrapper">
        ...
        : (
          <table>
            <thead>
              <tr><th>Proyecto</th><th>Área de Conocimiento</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              {datos.map((fila, i) => (
                <tr key={i}>
                  <td>{nombreProyecto(fila.proyecto)}</td>
                  <td>{nombreArea(fila.area_conocimiento)}</td>
        ...
        >
          ...
          <div className="form-grid">
            ...
            <div className="form-group full">
              ...
              <select name="area_conocimiento" value={form.area_conocimiento} onChange={handleChange}>
                <option value="">— Seleccionar —</option>
                {areas.map(a => <option key={a.id} value={a.id}>{a.id} - {a.disciplina}</option>)}
              </select>
            </div>
          </div>
        </Modal>
